...
export const removeItemFromCart = async (shoppingCartId: number, productId: number) => {
  const response = await fetch(`http://localhost:8080/api/v1/cart/removeItem/${shoppingCartId}/${productId}`, {
    method: "DELETE",
    headers: {
      "Content-Type": "application/json",
    },
